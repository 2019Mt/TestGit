...
public class Hero extends FlyingObject{
	...
	public void step(){ //10毫秒走一次
		image = images[index++/10%images.length]; //100毫秒切换一次
	...
	public Bullet[] shoot(){
		...
		if(doubleFire>0){ //双倍
			...
			bs[0] = new Bullet(this.x+1*xStep,this.y-20); //x:英雄机的x+1/4英雄机的宽  y:英雄机的y-20
			bs[1] = new Bullet(this.x+3*xStep,this.y-20); //x:英雄机的x+3/4英雄机的宽  y:英雄机的y-20
			doubleFire-=2; //发射一次双倍火力，则火力值减2
			return bs;
		}else{ //单倍
			Bullet[] bs = new Bullet[1];
			bs[0] = new Bullet(this.x+2*xStep,this.y-20); //x:英雄机的x+2/4英雄机的宽  y:英雄机的y-20			
			return bs;
		}
	}
